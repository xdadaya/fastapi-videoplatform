...
class TokenService:
    ...
    def verify_refresh_token(refresh_token: str) -> UUID:
        try:
            payload = jwt.decode(refresh_token, key=settings.SECRET_KEY, algorithms=settings.HASH_ALGORITHM)
        except Exception:
            raise HTTPException(status_code=403, detail='Authentication error. Unable to decode token')
        if payload["type"] == "refresh":
            return UUID(payload["id"])
        raise HTTPException(status_code=403, detail='Authentication error. Its not valid refresh token')
